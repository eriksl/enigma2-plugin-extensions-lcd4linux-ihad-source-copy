    ...
    def ical(self):
        if self.dt.tzinfo:
            if self.dt.year < 1900:
                self.dt = datetime(1900,self.dt.month,self.dt.day,self.dt.hour,self.dt.minute,self.dt.second,tzinfo=self.dt.tzinfo)
            utc_time = self.dt - self.dt.tzinfo.utcoffset(datetime.now())
            return utc_time.strftime("%Y%m%dT%H%M%SZ")
        if self.dt.year < 1900:
            self.dt = datetime(1900,self.dt.month,self.dt.day,self.dt.hour,self.dt.minute,self.dt.second)
...
class vDate:
    ...
    def ical(self):
        if self.dt.year < 1900:
            self.dt = date(1900,self.dt.month,self.dt.day)
